...
On first launch, a setup assistant explains the focus/warning/break loop and offers notification access, Calendar selection, live-meeting options, an Obsidian daily-notes folder, and launch at login. Every integration is optional, and setup can be skipped or run again from Settings. Notification permission is requested only after choosing **Enable Notifications** in setup or Settings.

The app otherwise lives quietly in the menu bar, where its status item shows the live countdown; Settings opens only when selected from the gear button. Clock in and let the countdown reach the warning window. The warning posts one notification when permitted and always plays a sound. At zero, start the break from the full-screen prompt, or choose **5 more minutes** in normal mode (**15 more seconds** in demo mode) to dismiss the prompt and begin a fresh warning. Break-return and activity prompts open centered in the current screen's visible area. Returning after lunch, travel, or another detected absence opens a quick prompt to update BreakBar before resuming focus. Starting a break makes a best-effort request to launch the macOS screensaver; returning to focus remains unavailable until the minimum break has elapsed.
...
Open **Today’s history** from the chart button in the menu-bar popover to see clocked-in time, working time, category totals, and the day’s activity timeline. Use the arrows to view earlier days. Ongoing intervals update in place, and activity crossing midnight is counted only within each displayed local day. Calendar, detected-call, manual, and offsite meetings are recorded separately from focus time. Select the active work session to correct its clock-in time; if its initial focus interval is still active, the countdown is recalculated from the corrected start. Completed work sessions allow both clock-in and clock-out corrections, and completed timeline entries allow category, start-time, and end-time corrections. If you forgot to clock out after travel and later resumed focus, select the completed travel entry and choose **End previous work session** with the time your day ended. BreakBar shortens the travel interval and keeps activity from the return onward in a separate work session. BreakBar rejects invalid or overlapping times, recalculates the summary immediately, and marks corrected entries as edited.
...
**Settings → Privacy & Data** explains what each integration can access. **Export Complete History…** writes every stored work session and activity to a versioned JSON archive. **Delete All Local History…** is available only while clocked out, requires confirmation, deletes the ledger records, attempts to clean their SQLite storage, reports a warning if that cleanup does not finish, and leaves preferences and Obsidian notes unchanged.

Open **Diagnostics** from the information button in the menu-bar popover to inspect timer, scheduling, integration, database, export, and BUSY Bar health. **Copy Diagnostics** produces a support report that omits calendar titles and identifiers, call application identifiers, file paths, device addresses, credentials, and raw errors.
