...
**Settings → Privacy & Data** explains what each integration can access. **Export Complete History…** writes every stored work session and activity to a versioned JSON archive. **Delete All Local History…** is available only while clocked out, requires confirmation, securely clears the SQLite ledger, and leaves preferences and Obsidian notes unchanged.
